...
func (w *MeshWebhook) validatePod(pod corev1.Pod) error {
	if _, ok := pod.Annotations[common.AnnotationProtocol]; ok {
		return fmt.Errorf("the %q annotation is no longer supported. Instead, create a ServiceDefaults resource (see www.consul.io/docs/k8s/crds/upgrade-to-crds)",
			common.AnnotationProtocol)
	}

	if _, ok := pod.Annotations[common.AnnotationSyncPeriod]; ok {
		return fmt.Errorf("the %q annotation is no longer supported because consul-sidecar is no longer injected to periodically register services", common.AnnotationSyncPeriod)
	}
	return nil
}
